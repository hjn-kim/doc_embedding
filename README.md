...
문자 밀도가 언어마다 다릅니다. 현재 코퍼스에서 가득 찬 510토큰 청크의 글자 수를 재면
이렇습니다.

| 언어 | 510토큰 청크의 글자 수(중앙값) | 자/토큰 |
|---|---|---|
| `ch` | 678 | 1.33 |
| `ko` | 852 | 1.67 |
| `en` | 1,648 | 3.23 |
...
**같은 500자라도 우즈베크어 청크가 중국어 청크보다 2.9배 큽니다.** 글자 수로 자르면
...
Precision·Recall·F1 은 쓰지 않습니다. 정답 청크가 문항당 1개 106 / 2개 40 / 3개 3 / 4개 1 로
대부분 1~2개라 `Recall@k` 는 `Hit@k` 와 사실상 같고, `Precision@k` 는 상한이 1/k 라 k 를
키우면 오히려 내려가서 @1 과 @3 을 나란히 놓고 읽을 수 없기 때문입니다.
